...
class DataProcessor:
    ...
    def _clean_and_filter(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Outlier Removal (only relevant for training data)
    ...
    def fit(self, df_raw: pd.DataFrame):
        """Learns imputation values and column structure from the training data."""
        ...
        df = self._clean_and_filter(df_raw)

        # Learn Imputation Values
        self.imputation_values = {
            'beds': df['beds'].median(), 'full_baths': df['full_baths'].median(), 'half_baths': 0,
            'lot_sqft': df['lot_sqft'].median(), 'year_built': df['year_built'].median(), 'hoa_fee': 0.0,
            'stories': df['stories'].mode()[0], 'parking_garage': df['parking_garage'].mode()[0],
            'neighborhoods': 'Unknown', 'text': "", 'list_price': df['list_price'].median(),
            'estimated_value': df['estimated_value'].median() # Global median as final fallback
        }
        ...
        print(f"--- DataProcessor fitting complete. Model will be trained on {len(self.training_columns)} features. ---")
    ...
    def transform(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.Series]:
        ...
        for col, term in self.config.POSITIVE_KEYWORD_MAP.items():
        ...
        df_transformed = pd.get_dummies(df_transformed, columns=categorical_features, prefix=categorical_features, dtype=int)
        ...
        if self.training_columns:
            ...
            # This path is taken during the .fit() method's "dry run"

            keyword_features = list(self.config.POSITIVE_KEYWORD_MAP.keys())
            ohe_features = [col for col in df_transformed.columns if any(cat in col for cat in categorical_features)]
            
            final_feature_list = self.config.BASE_FEATURES + keyword_features + ohe_features
